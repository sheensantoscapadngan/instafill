...
def detect_lines(pages_img):
    ...
    for i in range(len(pages_img)):
        ...
        detected_horizontal_lines = cv2.dilate(detected_horizontal_lines,horizontal_connect_kernel,iterations=1)
        detected_horizontal_lines = cv2.erode(detected_horizontal_lines,horizontal_connect_kernel,iterations=1)

        horizontal_cnts = cv2.findContours(detected_horizontal_lines,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
        ...
        detected_vertical_lines = cv2.morphologyEx(thresh, cv2.MORPH_OPEN, vertical_kernel, iterations=1)
        detected_vertical_lines = cv2.dilate(detected_vertical_lines, vertical_connect_kernel, iterations=1)
        detected_vertical_lines = cv2.erode(detected_vertical_lines, vertical_connect_kernel, iterations=1)

        vertical_cnts = cv2.findContours(detected_vertical_lines, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        vertical_cnts = vertical_cnts[0] if len(vertical_cnts) == 2 else vertical_cnts[1]
        ...
        boxes,line_to_box = detect_fillable_boxes(vertical_lines,horizontal_lines)
