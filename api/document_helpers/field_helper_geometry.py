...
def detect_fillable_boxes(vertical_lines,horizontal_lines):
    ...
    for i in range(n):
        for j in range(i+1,n):
            horizontal_l1 = LineString(horizontal_lines[i].retrieve_line_definition())
            horizontal_l2 = LineString(horizontal_lines[j].retrieve_line_definition())
            candidates = []
            for k in range(len(vertical_lines)):
                if len(candidates) > 1: continue
                vertical_l = LineString(vertical_lines[k].retrieve_line_definition())
            ...
            if len(candidates) > 1:
                ...
                for line in candidates:
                    line_to_box[line.retrieve_line_definition()] = len(box_list)
                box = Box(candidates[0],candidates[1],candidates[2],candidates[3])
                box_list.append(box)
