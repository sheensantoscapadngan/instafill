...
class Line:
    ...
    def compute_distance_to_point(self,point):
        num = abs((self.p2[0]-self.p1[0])*(self.p1[1]-point.y)-(self.p1[0]-point.x)*(self.p2[1]-self.p1[1]))
        denom = math.sqrt((self.p2[0]-self.p1[0])**2 + (self.p2[1]-self.p1[1])**2)
        dist = num/denom
...
class Box:
    ...
    def retrieve_bottom_line(self):
        ...
        for line in lines:
            ...
            if slope > SLOPE_THRESHOLD and line_def[0][1] > bottom_line_def[1]:
                bottom_line = line
